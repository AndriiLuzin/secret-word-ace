...
const WhoAmIPlayer = () => {
  ...
  const fetchGuesserCharacter = useCallback(async (gameId: string, guesserIdx: number) => {
    const { data } = await supabase
      .from("whoami_players")
      .select(`id, player_index, character_id, whoami_characters (name, category)`)
      .eq("game_id", gameId)
      .eq("player_index", guesserIdx)
      .maybeSingle();

    if (data) setGuesserPlayer({ ...data, character: (data as any).whoami_characters });
  }, []);

  const assignRole = useCallback(async (gameData: WhoAmIGame) => {
    const { data: existingViews } = await supabase.from("whoami_player_views").select("player_index").eq("game_id", gameData.id);
    ...
    const { error: insertError } = await supabase.from("whoami_player_views").insert({ game_id: gameData.id, player_index: availableIndex });
  ...
  const setupPlayer = useCallback(async (gameData: WhoAmIGame, idx: number) => {
    ...
    if (idx !== gameData.guesser_index) await fetchGuesserCharacter(gameData.id, gameData.guesser_index);
    setIsLoading(false);
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      if (existingIndex !== null) {
        ...
        const { data: view } = await supabase.from("whoami_player_views").select("player_index").eq("game_id", gameData.id).eq("player_index", idx).maybeSingle();
        if (view) { await setupPlayer(gameData, idx); return; }
      }

      const newIndex = await assignRole(gameData);
      if (newIndex !== null) await setupPlayer(gameData, newIndex);
    };

    init();
  }, [code, searchParams, assignRole, setupPlayer, t]);

  useEffect(() => {
    if (!game) return;

    const channel = supabase.channel(`whoami-player-${code}`)
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "whoami_games", filter: `id=eq.${game.id}` },
        async (payload) => {
          ...
          if (existingIndex !== null) {
            const idx = parseInt(existingIndex);
            const { data: view } = await supabase.from("whoami_player_views").select("player_index").eq("game_id", newGame.id).eq("player_index", idx).maybeSingle();
            if (!view) await supabase.from("whoami_player_views").insert({ game_id: newGame.id, player_index: idx });
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-background p-6 relative">
        ...
        <div className="text-center animate-fade-in">
          ...
          <Button onClick={() => setIsRevealed(true)} className="h-20 px-12 text-xl font-bold uppercase tracking-wider">{t.show}</Button>
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-background relative">
      ...
      <div className="text-center animate-scale-in">
        <p className="text-xs uppercase tracking-wider text-muted-foreground mb-4">
          {isGuesser ? t.yourTask : `${t.characterOfPlayer} #${(game?.guesser_index ?? 0) + 1}`}
        </p>
        <h1 className="text-4xl font-bold text-foreground">{isGuesser ? t.whoAmI : guesserPlayer?.character?.name}</h1>
        {!isGuesser && guesserPlayer?.character?.category && <p className="text-muted-foreground text-sm mt-2">{guesserPlayer.character.category}</p>}
        <p className="text-muted-foreground text-sm mt-6">
          {isGuesser ? (
            <>{t.youDontKnowCharacter}<br />{t.askYesNoQuestions}</>
          ) : (
            <>{t.player} #{(game?.guesser_index ?? 0) + 1} {t.playerDoesntKnowWhoHeIs}<br />{t.helpGuess}</>
          )}
        </p>
